$(function(){
	...
	var canvasWidth = parseInt($("#canvas").outerWidth()) + parseInt($(".simulation").outerWidth())*2;
	$(".up").click(function(){
	...
	var tableauUrl = "";
	/*模拟屏幕区*/
	var simulationLeft = $(".simulation").eq(0).offset().left;								 //定义模拟显示区左部坐标
	var simulationRight = simulationLeft + parseInt($(".simulation").eq(0).outerWidth())*2; 	//定义模拟显示区右部坐标
	var simulationBottom = boxTop + parseInt($(".simulation").eq(0).outerHeight());	//定义模拟显示区底部坐标
	var imgsrc = "";
	var showboxHeight = $(".simulation:eq(0) .showbox:first").outerHeight();
	...
	$(document).mousemove(function(event){
		if (isMouseDown)
		{
			...
			{
				var lineNum = parseInt((event.pageX-simulationLeft)/$(".simulation").eq(0).outerWidth())*$(".simulation").eq(0).find(".showbox").length;
				$(".showbox").removeClass("cur");
				$(".showbox").eq(parseInt((event.pageY - boxTop)/showboxHeight)+lineNum).addClass("cur");
				$(".tbox").removeClass("cur");
				$(".tbox").eq(parseInt((event.pageY - boxTop)/showboxHeight)+lineNum).addClass("cur");
			}else{
				$(".showbox").removeClass("cur");
			}
		}	
		...
		if(isMouseDown){
			...
			{
				var lineNum = parseInt((event.pageX-simulationLeft)/$(".simulation").eq(0).outerWidth())*$(".simulation").eq(0).find(".showbox").length;
				$(".showbox").eq(parseInt((event.pageY - boxTop)/showboxHeight)+lineNum).find("img").remove();
				$(".showbox").eq(parseInt((event.pageY - boxTop)/showboxHeight)+lineNum).append("<img src='"+imgsrc+"' />");
				$(".showbox").removeClass("cur");
				$(".tbox").eq(parseInt((event.pageY - boxTop)/showboxHeight)+lineNum).html("").removeClass("cur").append("<div class='loading-80'></div>");
				bigFunction(thisNo,parseInt((event.pageY - boxTop)/showboxHeight+lineNum),tableauUrl);
			}
		}
		//3840 2160
		//960 720
