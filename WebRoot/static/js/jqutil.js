 ...
 */
var mozi = mozi || {};
mozi.data = mozi.data || {};// 用于存放临时的数据或者对象
 ...
 * 增加命名空间功能
 * 
 * 使用方法：mozi.ns('jQuery.bbb.ccc','jQuery.eee.fff');
 */
mozi.ns = function() {
	var o = {}, d;
 ...
 * 将form表单元素的值序列化成对象
 * 
 * @example mozi.serializeObject($('#formId'))
 * @returns object
 */
mozi.serializeObject = function(form) {
	var o = {};
...
/**
 * 增加formatString功能
 * @example mozi.formatString('字符串{0}字符串{1}字符串','第一个变量','第二个变量');
 * @returns 格式化后的字符串
 */
mozi.formatString = function(str) {
 ...
 * @returns list
 */
mozi.stringToList = function(value) {
...
mozi.jsonToString = function(o) {
	...
	if (typeof o == "object") {
		if (!o.sort) {
			for ( var i in o)
				r.push(i + ":" + mozi.jsonToString(o[i]));
 ...
 * 
 * @example mozi.cookie('the_cookie', 'the_value');
 * @desc Set the value of a cookie.
 * @example mozi.cookie('the_cookie', 'the_value', { expires: 7, path: '/', domain: 'jquery.com', secure: true });
 * @desc Create a cookie with all available options.
 * @example mozi.cookie('the_cookie', 'the_value');
 * @desc Create a session cookie.
 * @example mozi.cookie('the_cookie', null);
 ...
 * 
 * @example mozi.cookie('the_cookie');
 ...
 * @author Klaus Hartl/klaus.hartl@stilbuero.de
 */
mozi.cookie = function(key, value, options) {
